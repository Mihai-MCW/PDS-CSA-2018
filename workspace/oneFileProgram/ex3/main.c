...
#define NB_MSG_TYPE   4   // Number of message types
...
sem_t empty, full, color[NB_MSG_TYPE-1], sem_display;
...
//	srand((int)pthread_self());
  ...
  int a;
  for(int i = 0; i< NB_MSG_TYPE; i++){
    initializeSemaphore(&color[i],0,0);
  }
  initializeSemaphore(&sem_display,0,1);
  pthread_mutex_init(&mutex,0);

  /* Creation of nbProd producers and nbConso consumers */
  for (i = 0; i <  nbThds; i++) {
    if (i < nbProd) {
      paramThds[i].typeOfMessage = i%NB_MSG_TYPE;
